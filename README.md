...
- 단체 신청의 참가자 이름은 신청자 이름과 동일하게 2~20자 기준으로 검증한다.
- 참가자 이메일은 이메일 형식과 중복 여부를 검증한다.
- 담당자 연락처는 신청자 전화번호와 동일하게 한국 전화번호 형식으로 검증한다.
- 단체 신청에서도 공통 신청자 정보는 신청을 진행하는 **대표자/담당자 정보**로 해석한다.
- 참가자 명단은 **실제 수강 대상자 목록**으로 분리하여 관리한다.

### 가정

- 대표자를 참가자 명단에 포함할지 여부는 서비스 정책에 따라 달라질 수 있다. 본 구현에서는 명세에 따라 신청자 정보와 참가자 명단을 **별도 데이터**로 관리한다.
...
- [ ] 반응형 레이아웃 (모바일 대응) — 기본적인 좁은 화면 레이아웃 보완은 일부 적용했으나, 별도 모바일 최적화는 미구현
