...
- [ ] 강의 선택 (카테고리 필터 + 강의 목록)
- [ ] 선택한 강의 정보 표시 (제목, 카테고리, 일정 등)
- [ ] 개인 / 단체 신청 유형 선택
- [ ] 공통 신청자 정보 입력 (이름, 이메일, 연락처)
- [ ] 단체 신청 조건부 필드 (단체명, 인원수)
- [ ] 참가자 명단 동적 입력 (신청 인원수에 맞춰 이름/이메일 입력 필드를 자동 생성)
- [ ] 확인 및 제출 화면 (입력 요약 표시)
- [ ] 약관 동의 (전체 동의 포함)
- [ ] 제출 성공 / 실패 상태 처리
- [ ] 스텝 이동 전 유효성 검증
- [ ] 스텝 간 입력 데이터 유지
- [ ] 스텝 인디케이터
- [ ] Mock API 구성 (강의 목록 GET, 신청 제출 POST)
...
```ts
type EnrollmentForm =
  | { type: "personal"; applicant: ApplicantInfo }
  | { type: "group"; applicant: ApplicantInfo; group: GroupInfo };
```

신청 유형에 따라 필드가 달라지므로 유니온 타입으로 정의하면 타입 가드를 통해 조건부 필드 접근이 안전해진다.
